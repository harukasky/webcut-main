<template>
  <header
    ...
    <div class="flex w-1/3 pl-2 items-center">
      ...
      <div class="ml-2">
        <img class="h-8" src="@/assets/ym2.ico" alt="">
      </div>
...
import { Combinator, type OffscreenSprite, renderTxt2ImgBitmap } from '@webav/av-cliper';
...
const trackState = useTrackState();//轨道状态
const playerState = usePlayerState();
const TrackAttr = useTrackAttrState();
...
const onGenerate = async () => {
  ...
  const sprs: Promise<OffscreenSprite>[] = [];//合成使用轨道数据合成，但是这样子的话，合成出来的视频不能够使用webAV再次打开
  //因为里面的mp4clip不支持多种编码格式（当前好像支持chrome102以上版本，放出来的编码格式）
  ...
  await com.output().pipeTo(await createFileWriter());//写成文件，让你选择，主要是这样子的话，为什么，不支持流媒体播放
...
async function postData() {
  axios({
    url: "http://dev.shujymei.com/main.php?c=lianxi2&s=getLivingData&zbzk=1",
    method: "POST",
    headers: {
